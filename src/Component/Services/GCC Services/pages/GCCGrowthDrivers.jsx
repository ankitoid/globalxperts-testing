          ...
          {cards.map((item, i) => (
            ...
            >
              ...
              <div className="relative z-10">
                ...
                {/* Title */}
                <h3 className="font-bold text-lg text-gray-900 mb-2 group-hover:text-gray-900">
                  {item.title}
                </h3>
              ...
              <div className={`absolute -bottom-4 -right-4 w-24 h-24 bg-gradient-to-br ${item.color} opacity-0 group-hover:opacity-10 rounded-full blur-2xl transition-opacity`}></div>
            </div>
          ))}
        </div>

        {/* Wide Bottom Card - Featured */}
        <div className="relative group bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl shadow-2xl hover:shadow-3xl p-8 transition-all duration-300 overflow-hidden">
          ...
          <div className="relative z-10 flex flex-wrap gap-2 mt-6 pt-6 border-t border-white/20">
            {['Cloud Native', 'AI/ML', 'Microservices', 'DevOps', 'Data Platforms'].map((tech, i) => (
              <span key={i} className="text-xs bg-white/10 backdrop-blur-sm text-white px-3 py-1.5 rounded-full border border-white/20 hover:bg-white/20 transition-colors">
                {tech}
              </span>
            ))}
          </div>
          ...
          {/* Glow effect */}
          <div className="absolute -inset-2 bg-gradient-to-r from-blue-600 to-purple-600 opacity-0 group-hover:opacity-20 blur-2xl transition-opacity -z-10"></div>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-700 leading-relaxed">
                The GCC sector in India is projected to grow to <span className="font-semibold text-green-700">$100B+</span> by 2030, 
                driven by these key factors and the increasing strategic importance of innovation centers.
              </p>
            </div>
          </div>
        </div>
